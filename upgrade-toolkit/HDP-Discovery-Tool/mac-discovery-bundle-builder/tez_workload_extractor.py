...
def _make_tarfile_tez(output_file_path, source_dir):
    log.info("Creating tarball at " + output_file_path + " from " + source_dir + "/done")
    files = os.listdir(source_dir+"/done")
    with tarfile.open(output_file_path, "w:gz") as tar:
        for f in files:
            if f in ('app_data', 'dag_meta', 'dag_data'):
                tar.add(os.path.join(source_dir, "done", f), os.path.basename(f))
    log.info("Tarball created at: %s" % output_file_path)

def _make_tarfile_hive_query_data(output_file_path, source_dir):
    log.info("Creating tarball at " + output_file_path + " from " + source_dir + "/done")
    files = os.listdir(source_dir+"/done")
    with tarfile.open(output_file_path, "w:gz") as tar:
        for f in files:
...
class TezHistoryExtractor:
    ...
    def collect_tez_logs_for_cluster(self, hdfs_url, download_dir):
        default_history_dir = "/warehouse/tablespace/external/hive/sys.db"
        ...
        if "KERBEROS" in service_list:
            ...
            try:
                ...
                log.error("Issue with webhdfs downloading " + default_history_dir)
                log.error(e)
        else:
            ...
            try:
                ...
                log.error(e)
        try:
            _make_tarfile_tez(os.path.join(download_dir, "tez-history.tar.gz"), download_dir)
            _make_tarfile_hive_query_data(os.path.join(download_dir, "hive-query_data.tar.gz"), download_dir)
        except Exception as e:
            log.error("Issue with taring the directory " + download_dir)
            log.error(e)

    def collect_tez_histories(self):
        ...
        if 'NAMENODE' in master_components:
            ...
            if hdfs_config['web_hdfs_enabled'] == "true":
                ...
                tez_output_dir = os.path.join(self.output_dir, "workload", "TEZ")
                create_directory(tez_output_dir)
                self.collect_tez_logs_for_cluster(active_namenode_url, tez_output_dir)
            else:
                log.debug("Webhdfs is not enabled, Unable to collect the tez job history via webhdfs")
    ...
    def collect_hdfs_config(self):
        ...
        for config_type in config_types:
            if config_type['type'] == 'hdfs-site':
                hdfs_config['ns'] = config_type['properties']['dfs.nameservices']
