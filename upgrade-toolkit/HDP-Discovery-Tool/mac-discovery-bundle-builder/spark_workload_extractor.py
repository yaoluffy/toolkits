...
class SparkHistoryExtractor:
    ...
    def collect_spark_logs_for_cluster(self, hdfs_url, download_dir):
        default_history_dir = "/spark2-history"
        ...
        if "KERBEROS" in service_list:
            ...
            try:
                ...
                log.error(e)
        try:
            _make_tarfile(os.path.join(download_dir, "spark2-history.tar.gz"), download_dir)
        except Exception as e:
            log.error("Issue with taring the directory " + download_dir)
            log.error(e)

    def collect_spark_histories(self):
